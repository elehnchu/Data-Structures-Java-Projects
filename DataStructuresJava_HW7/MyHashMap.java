...
public class MyHashMap<K extends Comparable<K>, V> implements MyMap<K, V> {
    ...
    //primeindex used to get the index or primes; numEntries = number of entries in Hashmap
    ...
    @Override
    public V get(K key) {
        //calculate the key were looking for
        int index = key.hashCode() % getTableSize();
        Entry<K, V> current= table[index];
        //now we iterate through linked list
        while (current != null){
            //use .equals for nonprimitive data types
    ...
    public V put(K key, V value) {
        ...
        if (table[index] == null){
            ...
            //current is now our head of linked list
            Entry<K, V> current= table[index];
            //now we iterate through linked list
            while (current != null){
                //use .equals for nonprimitive data types
    ...
    private void rehash() {
        primeIndex++;
        //copying table onto this table
        Entry<K, V>[] oldtable= table;
        //now table is next index size
        table = new Entry[primes[primeIndex]];
        numEntries=0;
    ...
    public V remove(K key) {
        ...
        if (table[index] == null || current == null){
            return null;
        }
        if (prev == null){
